...
def farm_block(
    ...
    hkp = header_signature.aggsig_pair(proof_of_space.plot_public_key, HeaderHash(header))
...
def test_farm_block_empty():
    ...
    additions, removals = run(chain_view.accept_new_block(header, unspent_db, REWARD, 0))
...
def test_farm_two_blocks():
    ...
    chain_view = run(chain_view.augment_chain_view(
        header, header_signature, unspent_db, unspent_db, REWARD, 0))
